...
HPEN hOnPen,hOffPen,hPausePen;						// Pens for painting LEDs
BOOL bRunning;
...
hOnPen = CreatePen(PS_SOLID,4,RGB(0,0,0));    	// Create LED Draw Pens
...
switch(iMessage)
	...
	case WM_SIZE:											// Window resized
		...
		xBorder = x / 40;yBorder = y/40;
		rcFrame.left = xBorder;rcFrame.right = x - xBorder;
		rcFrame.top = yBorder;rcFrame.bottom = y - yBorder;
		...
		for (i = 0;i < DIGITS;i++)
			...
			rcLED[i].bottom = rcFrame.bottom - x1;
			}
		break;
	...
	case WM_PAINT:											// Paint window
		...
		MoveTo(ps.hdc,rcFrame.right,rcFrame.top);
...
#define LINE(b,x1,y1,x2,y2) { SelectObject(hDC,b & New ? hUsePen : hOffPen);MoveTo(hDC,x1,y1);LineTo(hDC,x2,y2); }
